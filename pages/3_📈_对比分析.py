...
from queries.results import search_participants, get_competitions
...
with tab1:
    ...
    name_search = st.text_input("搜索选手姓名", key="compare_search",
                                placeholder="输入姓名...")

    if name_search:
        candidates = search_participants(name_search)
        if not candidates.empty:
            options = {
                row['id']: f"{row['name']}（{row['district']}）"
            ...
            if len(selected_ids) >= 2:
                ...
                comp_options = {0: '全部比赛'}
                comp_options.update({
                    row['id']: row['name'] for _, row in comps.iterrows()
                })
...
with tab2:
    ...
    name_search2 = st.text_input("搜索选手", key="progression_search",
                                 placeholder="输入姓名...")

    if name_search2:
        candidates2 = search_participants(name_search2)
        if not candidates2.empty:
            ...
            selected_id = st.selectbox(
                "选择选手",
                options=list(options2.keys()),
                format_func=lambda x: options2[x],
                        ...
                        for event in events_to_show:
                            ...
                            if len(edata) >= 1:
                                ...
                                with cols[0]:
                                    ...
                                    # Show delta if multiple competitions
                                    if len(edata) >= 2:
